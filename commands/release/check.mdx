...
| `1` | One or more gates are blocking (`fail` with high/critical severity or raw `fail`) |
| `2` | One or more gates produced a non-blocking `warn` or `waived` result |
| `3` | Configuration error (missing or invalid `cops.project.json`) |
